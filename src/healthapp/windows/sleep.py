...
class Sleep():
    ...
    def get_content(self) -> toga.Box:
        ...
        header_label = toga.Label(
            "Sleep Schedule: ", style=Pack(font_size=15, padding=(0, 10)))

        s_label = toga.Label("How many hours of sleep do you get?", style=Pack(
            font_size=15, padding=(0, 10)))

        self.s_text_input = toga.TextInput(placeholder='hours', style=Pack(
            background_color="#fbf5cc"), value=self.app.user.sleep)
        s_text_input_box = create_border(
            self.s_text_input, inner_color="#fbf5cc")

        submit_button = toga.Button('Submit', on_press=self.sleep_handler, style=Pack(

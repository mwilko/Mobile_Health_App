...
# Create a border around most input fields (15px total, 13px outside black border, 2px black border)
def create_border(object, border_color="black", inner_color="#e2985f", padding=(13, 13, 0)) -> toga.Box:
    ...
    in_box = toga.Box(style=Pack(direction=COLUMN, background_color=inner_color, padding=(2)))
    in_box.add(object)
    out_box.add(in_box)
    return out_box
